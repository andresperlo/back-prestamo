        ...
        for (let index = 0; index < file.length; index++) {
            ...
            if (element) {
                ...
                try {
                    ...
                    res.status(500).send({ message: 'error' + error + pathFile})
                }


            } else {
                res.status(500).send({ message: 'No existe Element PDF' + ' path ->', pathFile })
            }

        }

    } catch (error) {
        res.status(500).send({ mensaje: 'error' + error + pathFile})
    }
